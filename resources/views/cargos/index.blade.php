        ...
        <table class="table table-hover">
            ...
            <tbody>
                @forelse ($cargos as $cargo)
                    <tr>
                        <td class="text-center align-middle">{{ $cargo->id_cargo }}</td>
                        <td class="align-middle">{{ $cargo->nombre_cargo }}</td>
                        <td class="align-middle">{{ $cargo->siglas_cargo }}</td>
                        <td class="text-center align-middle">
                            <div class="d-flex justify-content-center">
                                <a href="{{ route('cargos.edit', $cargo->id_cargo) }}" class="btn btn-sm btn-warning mx-1">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('cargos.destroy', $cargo->id_cargo) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-danger mx-1" onclick="return confirm('¿Está seguro de eliminar este cargo?')">
